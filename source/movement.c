...
int movement_init(){
	...
	int rets=ev3_search_sensor( HT_NXT_COMPASS, &compass_sn, 0 );
	if (rets) {
		printf("FOUND IT\n");
	}
	else {
	...
	float a=0;
	for (int i = 0; i < 10; ++i) {
		get_sensor_value0(compass_sn, &ang_off);
		a+=ang_off;
		Sleep(100);
	}
	ang_off=a/10;
...
void TURN_DEGREES_COMP(int ang_speed, int angle)
{
	int i=0;
	float dest;
	float buffer[4];
	...
	float avg=0;
	for (i = 0; i < 4; ++i) {
		get_sensor_value0(compass_sn,&buffer[i]);
		/*printf("COMPASS VALUE before TURN : %f\n", buffer[i]);*/
	}
	for (i = 0; i < 4; ++i) {
		avg+=buffer[i];
	}
	...
	j=0;
	while (1) {
		get_sensor_value0(compass_sn,&buffer[j%4]);
		avg=0;
		for (i = 0; i < 4; ++i) {
			avg+=buffer[i];
		}
		avg=avg/4;
		j++;
		/*printf("COMPASS VALUE is %f\n", avg);*/
		if (fabs(avg-dest)<=1) {
			/*printf("FINAL COMPASS VALUE is %f\n", avg);*/
...
void* tracepos()
{
	...
	void forward(){
		set_tacho_speed_sp(motor[L], MAX_SPEED  / 5 );
		set_tacho_speed_sp(motor[R], MAX_SPEED / 5 );
	...
	void movement_start() {
		...
		/*TURN_DEGREES_COMP(ANGULAR_SPEED*1/4, 90);*/
		/*Sleep(10000);*/
		/*TURN_DEGREES(ANGULAR_SPEED, -90*angparam/2.08);*/
		/*TURN_DEGREES_COMP(ANGULAR_SPEED, 90);*/
		/*Sleep(5000);*/
		/*angparam-=0.05;*/
		/*}*/
		/*stop();*/
		...
		for (int i = 0; i < 4; ++i) {
			...
			/*Sleep(5000);*/
			TURN_DEGREES_COMP(ANGULAR_SPEED/4, 90);
			/*Sleep(5000);*/
